...
import { iUserFans, iUserFriend, iUserFriendName,iUser,iSystemRegion,iSystemLanguage } from "~/server/db/schema";
...
export const relationRouter = createTRPCRouter({
  ...
  getFollowList: publicProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const follows = await ctx.db.query.iUserFans.findMany({
        ...
        orderBy: [desc(iUserFans.insertTime)]
            ...
            // 获取所有需要的region IDs
    const regionIds = follows.map(follow => follow.fansUser.region).filter(Boolean);
    ...
    const regions = await ctx.db.query.iSystemRegion.findMany({
      ...
      const enrichedFans = follows.map(follow => ({
        ...
        fansUser: {
          ...follow.fansUser,
          regionName: follow.fansUser.region ? regionMap[follow.fansUser.region] : undefined
        }
      }));
      // 获取总数
    ...
    .query(async ({ ctx, input }) => {
      ...
      const fans = await ctx.db.query.iUserFans.findMany({
        ...
        orderBy: [desc(iUserFans.insertTime)]
          ...
          // 获取所有需要的region IDs
    const regionIds = fans.map(fan => fan.user.region).filter(Boolean);
  ...
  getFriendList: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const regionIds = friends.map(friend => friend.friendUser.region).filter(Boolean);
    ...
    const regions = await ctx.db.query.iSystemRegion.findMany({
      ...
      const languageIds = friends.map(friend => friend.friendUser.language).filter(Boolean);
      ...
      const enrichedFans = friends.map(friend => ({
        ...
        friendUser: {
          ...friend.friendUser,
          regionName: friend.friendUser.region ? regionMap[friend.friendUser.region] : undefined,
          languageName: friend.friendUser.language ? languageMap[friend.friendUser.language] : undefined
        }
      }));
  ...
  handleFriendRequest: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      if (input.accept) {
        ...
        // 拒绝好友请求
        await ctx.db.update(iUserFriend)
          .set({
            status: 4, // 主动拒绝
            updateTime: new Date()
          })
  ...
  removeFriend: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      ...
      await ctx.db.transaction(async (tx) => {
        ...
        // 删除好友备注
        await tx.delete(iUserFriendName)
          .where(
            and(
  ...
  getFriendRequests: protectedProcedure
    ...
    .query(async ({ ctx, input }) => {
      ...
      const enrichedRequests = friendRequests.map(req => ({
        ...
        regionName: req.user.region 
          ? regionMap[req.user.region] || '' 
          : '',
