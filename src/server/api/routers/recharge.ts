        ...
        try {
          ...
          await tx.update(iUserAccount)
            .set({
              ...
              totalAmount: userAccount.totalAmount + input.amount,
            })
            .where(eq(iUserAccount.userId, order.buyUserId));
  ...
  rechargeGoldCoin: protectedProcedure
    ...
    .mutation(async ({ ctx, input }) => {
      return await ctx.db.transaction(async (tx) => {
        try {
          // 1. 获取并验证订单
          const order = await tx.query.iRecordGoldCoin.findFirst({
            where: and(
          ...
          await tx.update(iUserAccount)
            .set({
              goldCoin: userAccount.goldCoin + input.goldCoin,
              totalAmount: userAccount.totalAmount + input.amount,
            })
            .where(eq(iUserAccount.userId, order.buyUserId));
    ...
    .mutation(async ({ ctx, input }) => {
      return await ctx.db.transaction(async (tx) => {
        try {
          ...
          await tx.update(iUserAccount)
            .set({
              character: userAccount.character + input.character,
              totalAmount: userAccount.totalAmount + input.amount,
            })
            .where(eq(iUserAccount.userId, order.buyUserId));
