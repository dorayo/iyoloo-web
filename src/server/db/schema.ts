...
export const iUserFriendProfit = mysqlTable(
  "i_user_friend_profit",
  {
    ...
    profitAmount: decimal("profit_amount", { precision: 10, scale: 2 }).default(
      "0.01",
    ),
    insertTime: timestamp("insert_time").default(sql`CURRENT_TIMESTAMP`),
    updateTime: timestamp("update_time")
...
// 账单类型码描述表
export const iRecordBillTypeCodeDescribe = mysqlTable(
  "i_record_bill_type_code_describe",
  {
    id: int("id").primaryKey().autoincrement(),
    brand: varchar("brand", { length: 50 }).default("iyoloo"),
    typeCode: int("type_code"),
...
export const iRecordGoldCoinRelations = relations(
  iRecordGoldCoin,
  ({ one }) => ({
    buyer: one(iUser, {
...
export const iRecordRedEnvelopesRelations = relations(
  iRecordRedEnvelopes,
  ({ one }) => ({
    giver: one(iUser, {
...
export const iRecordTranslateRelations = relations(
  iRecordTranslate,
  ({ one }) => ({
    buyer: one(iUser, {
...
export const iRecordVipNotReceivedRelations = relations(
  iRecordVipNotReceived,
  ({ one }) => ({
    user: one(iUser, {
...
// 添加账单类型关系
export const iRecordBillTypeCodeRelations = relations(
  iRecordBillTypeCode,
  ({ many }) => ({
...
export const iRecordBillTypeCodeDescribeRelations = relations(
  iRecordBillTypeCodeDescribe,
  ({ one }) => ({
...
export const iMessageSystemUser = mysqlTable(
  "i_message_system_user",
  {
    id: int("id").primaryKey().autoincrement(),
    brand: varchar("brand", { length: 50 }).default("iyoloo"),
...
// User message table
export const iMessageUser = mysqlTable(
  "i_message_user",
  {
    id: int("id").primaryKey().autoincrement(),
    brand: varchar("brand", { length: 50 }).default("iyoloo"),
...
export const iMessageSystemUserRelations = relations(
  iMessageSystemUser,
  ({ one }) => ({
